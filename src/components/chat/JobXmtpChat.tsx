...
import {
    Loader2, Send, Search, Paperclip, Smile, ShieldCheck, Mic, Trash2,
    Bold, Italic, Strikethrough, Link, List, ListOrdered, Code, Type, AtSign,
    Plus, LayoutList, ChevronDown, Video, FileText, X, CheckCircle,
    AlertTriangle, Clock, Target, Scale, Printer, Hash,
} from "lucide-react";
...
export default function JobXmtpChat({
    ...
    const {
        messages, draft, setDraft, isInitializing, isRefreshing, isSending,
        error, setError, statusHint, canChatByStatus, isParticipant,
        normalizedCurrent, counterPartyAddress, handleSend, handleSendAudio, initializeChat,
    } = useXmtpChat({ currentUserAddress, clientAddress, freelancerAddress, jobStatus });

    const { isRecording, recordingDuration, startRecording, stopRecording, cancelRecording, formatDuration, cleanup } =
        useVoiceRecorder({
    ...
    const meetRoomName = `fairwork-${clientAddress.toLowerCase().slice(2, 8)}-${freelancerAddress.toLowerCase().slice(2, 8)}`;
            ...
            const res = await fetch("/api/ai/transcript", {
                ...
                body: JSON.stringify({
                    messages: messages.map((m) => ({
                        text: m.text, isAudio: !!m.isAudio, audioUrl: m.audioUrl || null,
                        isMine: m.isMine, sentAt: m.sentAt,
                    })),
                    jobTitle, jobAmount,
                }),
            });
            if (!res.ok) throw new Error((await res.json()).error || "Failed to generate transcript");
    ...
    }, [jobAmount, jobTitle, messages]);

    const generateLegalReport = useCallback(async () => {
        ...
        try {
            const res = await fetch("/api/ai/legal-report", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ transcript: transcriptData, jobTitle, jobAmount, clientAddress, freelancerAddress }),
            });
            if (!res.ok) throw new Error((await res.json()).error || "Failed to generate legal report");
            const data = await res.json();
    ...
    }, [clientAddress, freelancerAddress, jobAmount, jobTitle, transcriptData]);
    ...
    const handlePrintReport = useCallback(() => {
        ...
        </head><body>${legalReport}</body></html>`);
        printWindow.document.close();
        printWindow.print();
    }, [jobTitle, legalReport]);

    if (!canChatByStatus) {
        return <p className="text-sm text-[#8888a0]">Chat becomes available after a freelancer accepts this job.</p>;
    }
    if (!currentUserAddress) {
        return <p className="text-sm text-[#8888a0]">Connect your wallet to access chat.</p>;
    }
    if (!isParticipant) {
        return <p className="text-sm text-[#8888a0]">Only the job client and accepted freelancer can access this chat.</p>;
    }
    ...
    return (
        <div className="h-full flex border-0 overflow-hidden font-sans">
            ...
            <section className="flex-1 flex flex-col bg-white min-w-0 h-full relative">
                {/* Header */}
                <div className="px-4 py-2 border-b border-[#e5e5ea] bg-[#fafafa] flex items-center justify-between shadow-sm shrink-0">
                    ...
                    <div className="flex items-center gap-2 text-[#999]">
                        ...
                        <button onClick={() => setMeetOpen(true)} title="Start video meeting"
                            ...
                            <Video className="w-3.5 h-3.5" /><span className="hidden sm:inline">Meet</span>
                        </button>
                        <button onClick={generateTranscript} disabled={isGeneratingTranscript || messages.length === 0}
                ...
                <div className="flex-1 overflow-y-auto p-4 min-h-0">
                    {isInitializing ? (
                        <div className="h-full flex items-center justify-center text-sm text-[#999] gap-2">
                            <Loader2 className="w-4 h-4 animate-spin" /> Initializing XMTP chat...
                        </div>
                    ) : error ? (
                        <div className="space-y-3">
                            <div className="text-sm text-red-500">{error}</div>
                            <button onClick={() => void initializeChat()}
                        ...
                        <div className="space-y-4">
                            {messages.map((message) => (
                                <div key={message.id} className="flex gap-3 items-start group hover:bg-[#f5f5f8] p-1.5 -mx-1.5 rounded-lg transition-colors">
                                    <div className="w-10 h-10 shrink-0 mt-0.5 rounded flex items-center justify-center font-bold text-white shadow-sm"
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-baseline gap-2">
                                            ...
                                            <span className="text-[11px] text-[#999]">{formatTime(message.sentAt)}</span>
                                        </div>
                                        <div className="text-[#333] text-[15px] leading-relaxed break-words mt-0.5">
                                            {message.isAudio ? (
                                                <audio controls src={message.audioUrl} className="max-w-[400px] w-full h-10 rounded outline-none mt-1" />
                                            ) : (
                                                <p>{message.text}</p>
                                            )}
                                        </div>
                ...
                <div className="px-4 pb-4 pt-1 bg-white w-full shrink-0">
                    <div className="border border-[#e5e5ea] rounded-xl bg-white flex flex-col focus-within:border-[#6366f1] transition-all shadow-sm">
                        {/* Formatting toolbar */}
                        <div className="flex items-center gap-1 border-b border-[#e5e5ea] px-2 py-1.5 text-[#999] overflow-x-auto">
                        ...
                        <div className="min-h-[56px] flex items-center w-full px-3 py-2">
                            {isRecording ? (
                                ...
                                <input
                                    ...
                                    onKeyDown={(e) => {
                                        if (e.key === "Enter" && !isSending && !isInitializing && !error && draft.trim()) {
                                            e.preventDefault();
                                            handleSend();
                                        }
                                    }}
                        ...
                        <div className="flex items-center justify-between px-2 py-1.5">
                            <div className="flex items-center gap-1 text-[#999]">
                                <button className="p-1.5 rounded-full hover:bg-[#f0f0f4] transition-colors"><Plus className="w-4 h-4" /></button>
                                <button className="p-1.5 rounded-full hover:bg-[#f0f0f4] hidden sm:block transition-colors"><Type className="w-4 h-4" /></button>
                                <button className="p-1.5 rounded-full hover:bg-[#f0f0f4] transition-colors"><Smile className="w-4 h-4" /></button>
                                <button className="p-1.5 rounded-full hover:bg-[#f0f0f4] transition-colors"><AtSign className="w-4 h-4" /></button>
                                {!draft.trim() && !isRecording && (
                                    <>
                                        <button className="p-1.5 rounded-full hover:bg-[#f0f0f4] transition-colors"><Paperclip className="w-4 h-4" /></button>
                                        <button onClick={startRecording} className="p-1.5 rounded-full hover:bg-[#f0f0f4] transition-colors"><Mic className="w-4 h-4" /></button>
                                    </>
                                )}
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                {isRecording ? (
                                    ...
                                    <button onClick={handleSend}
                                        ...
                                        className={`w-8 h-8 rounded shrink-0 flex items-center justify-center transition-colors ${draft.trim() && !isInitializing && !error && !isSending ? "bg-[#d4a017] text-white hover:bg-[#c59515]" : "bg-[#f0f0f4] text-[#ccc] cursor-not-allowed"}`}>
                                            ...
                                            <ul className="space-y-2 ml-10">
                                                {transcriptData.decisions.map((d: string, i: number) => (
                                                    <li key={i} className="flex items-start gap-2 text-[13px] text-[#555]">
                                                        <span className="text-[#3b82f6] mt-0.5">•</span><span>{d}</span>
                                                    </li>
                                                ))}
                                            </ul>
                                        </div>
                                            ...
                                            <div className="rounded-xl bg-[#f5f3ff] border border-[#ede9fe] p-4">
                                                ...
                                                <ul className="space-y-2.5 ml-1">
                                                    {transcriptData.timeline.map((t: string, i: number) => (
                                                        <li key={i} className="flex items-start gap-2.5 text-[13px] text-[#555]">
                                                            <div className="w-1.5 h-1.5 rounded-full bg-[#8b5cf6] mt-1.5 shrink-0" /><span>{t}</span>
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
                        ...
                        {transcriptData && !isGeneratingTranscript && (
                            <div className="px-5 py-3 border-t border-[#e5e5ea] bg-[#fafafa] rounded-b-2xl flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={generateLegalReport} disabled={isGeneratingReport}
                        ...
                        <div className="flex-1 overflow-y-auto px-10 py-8 bg-white">
                            {isGeneratingReport ? (
                                ...
                                <>
                                    ...
                                    `}</style>
                                    <div className="legal-report-content max-w-none"
                        ...
                        {legalReport && !isGeneratingReport && (
                            <div className="px-5 py-3 border-t border-[#e0e0e0] bg-[#f8f8fa] rounded-b-2xl flex items-center justify-between">
                                ...
                                <button onClick={generateLegalReport} className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-[#f0f0f4] border border-[#ddd] text-[#6366f1] text-[12px] font-medium hover:bg-[#e8e8ec] transition-colors">
                                    <Scale className="w-3 h-3" /> Regenerate
                                </button>
                            </div>
                        )}
                    </div>
            ...
            {meetOpen && (
                <JitsiMeetModal
                    ...
                    isModerator={currentUserAddress?.toLowerCase() === clientAddress.toLowerCase()}
